...
def naming(screen):
    ...
    p1 = py.transform.scale_by(p1, 1.5)
    p2 = py.transform.scale_by(p2, 1.5)
    w = p2.get_width()
    h = p2.get_height()

    vs = py.image.load("Media/versus1.png")
    vs = py.transform.scale_by(vs, 0.75)
    vs_rect = vs.get_rect()
    ...
    input_box1.center = (100+w/2, settings.height/2-200)
    input_box2 = py.Rect(10, 10, box_width, box_height)
    input_box2.center = (settings.width-100-w/2, settings.height/2+200)
    ...
    next = py.Rect(10, 10, 300, 100)
    next.topleft = (popup.x+50, popup.y+260)
    back = py.Rect(10, 10, 300, 100)
    ...
    while True:
        ...
        if (done == [True, True]):
            ...
            popup_heading = py.transform.scale_by(popup_heading, 0.7)
            popup_heading_rect = popup_heading.get_rect()
            popup_heading_rect.center = (settings.width/2, popup.y+50)
            ...
            name1_rect.x = name2_rect.x = popup.x + 40
            name1_rect.y = popup.y + 100
            name2_rect.y = name1_rect.y + 80

            

            nextText = settings.bigFont.render("CONTINUE", True, (0,0,0))
            nextText = py.transform.scale_by(nextText, 0.5)
            nextText_rect = nextText.get_rect()
            nextText_rect.center = (next.x+150, next.y+50)

            backText = settings.bigFont.render("BACK", True, (0,0,0))
            backText = py.transform.scale_by(backText, 0.5)
            backText_rect = backText.get_rect()
            backText_rect.center = (back.x+150, back.y+50)
